...
class _ProfilePageState extends State<ProfilePage> {
  ...
  bool get isFriend => _acceptedFriendRequests.contains(widget.name);
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            ElevatedButton.icon(
              onPressed:
                  (_sentFriendRequests.contains(widget.name) && !isFriend)
                  ? null
                  : () async {
                      if (isFriend) {
                        setState(() {
                          _acceptedFriendRequests.remove(widget.name);
                        });
                        await _saveAcceptedFriends();
                        ...
                        setState(() {
                          _sentFriendRequests.add(widget.name);
                        });
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
              ...
              icon: Icon(
                isFriend
                    ? Icons.people
                    : (_sentFriendRequests.contains(widget.name)
                          ? Icons.schedule
                          : Icons.person_add),
              ),
              label: Text(
...
Future<void> _saveAcceptedFriends() async {
  final prefs = await SharedPreferences.getInstance();
  await prefs.setStringList(
    "accepted_friends",
    _acceptedFriendRequests.toList(),
  );
}

Future<void> _loadAcceptedFriends() async {
  final prefs = await SharedPreferences.getInstance();
  final saved = prefs.getStringList("accepted_friends") ?? [];
  _acceptedFriendRequests
    ..clear()
    ..addAll(saved);
}
...
class _HomePageState extends State<HomePage> {
  ...
  void _listenToOnlinePosts() {
    FirebaseFirestore.instance
        ...
        .listen((snapshot) {
          ...
          final onlinePosts = snapshot.docs.map((doc) {
            final data = doc.data();
          ...
          setState(() {
            posts.removeWhere((post) => post['online'] == true);
            posts.insertAll(0, onlinePosts);
          });
        });
  }

  Future<void> _saveRefusedFriends() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setStringList(
      'declined_friends',
      _declinedFriendRequests.toList(),
    );
  }

  Future<void> _loadRefusedFriends() async {
    final prefs = await SharedPreferences.getInstance();
    final saved = prefs.getStringList('declined_friends') ?? [];
    _declinedFriendRequests
      ..clear()
      ..addAll(saved);
    if (mounted) setState(() {});
  }
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          if (post['video'] != null) PostVideoPlayer(path: post['video']),
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Row(
              children: [
                ...
                const SizedBox(width: 15),
                const Icon(Icons.share, color: Colors.purple, size: 23),
                const SizedBox(width: 8),
                Text('${post['shares'] ?? 0}'),
              ],
            ),
  ...
  Widget _simplePage(IconData icon, String title) {
    if (title == 'Friends' || title == 'My Friends') {
      final friends = title == 'My Friends'
          ? _acceptedFriendRequests.toList()
          : {'Rahul Das', 'Suman Ghosh', 'Amit Roy'}
                .where(
                  (name) =>
                      _acceptedFriendRequests.contains(name) == false &&
                      _declinedFriendRequests.contains(name) == false,
                )
                .toList();
                                ...
                                child: OutlinedButton(
                                  onPressed: () {
                                    setState(() {
                                      _declinedFriendRequests.add(name);
                                      _saveRefusedFriends();
                                      _sentFriendRequests.remove(name);
                                    });
                                  },
                                  child: const Text('Refuse'),
                                ),
                ...
                ListTile(
                  ...
                  onTap: () {
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (_) =>
                            _simplePage(Icons.people_outline, 'My Friends'),
                      ),
                    );
                  },
                ),
  ...
  Future<void> _pickVideo() async {
    ...
    final ref = FirebaseStorage.instance.ref().child(
      "post_videos/${DateTime.now().millisecondsSinceEpoch}.mp4",
    );
      ...
      builder: (sheetContext) => SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ListTile(
                leading: const Icon(Icons.edit, color: Color(0xFFD81B60)),
              ...
              ListTile(
                ...
                onTap: () {
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (_) => const WatchLivePage()),
                  );
                },
              ),
              ListTile(
